...
// Helper function to get the initialized AI client using the key from local storage
const getAiClient = (): GoogleGenAI => {
  const apiKey = localStorage.getItem("gemini-api-key");
  if (!apiKey) {
    // This is a safeguard. The UI should prevent this from being called without a key.
    alert(
      "Gemini API Key not found. Please refresh the page and enter your key.",
    );
...
const createErrorMessage = (
  ...
  if (response.promptFeedback?.blockReason) {
    ...
    const safetyDetails = response.candidates[0].safetyRatings
      ?.map(
        (rating) =>
          `${rating.category.replace("HARM_CATEGORY_", "")}: ${rating.probability}`,
      )
...
export const generatePromptFromMusic = async (
  audioBase64: string,
  mimeType: string,
  context?: string,
): Promise<string> => {
  try {
    ...
    if (!response.text) {
      throw new Error(
        createErrorMessage(
          response,
          "Model failed to return a valid prompt from the audio.",
        ),
      );
    }
    ...
    throw new Error(
      error.message || "Failed to analyze music. Please try another file.",
    );
  }
};

export const transcribeAudioToPrompt = async (
  audioBase64: string,
  mimeType: string,
  context?: string,
): Promise<string> => {
  try {
    ...
    if (!response.text) {
      throw new Error(
        createErrorMessage(
          response,
          "Model failed to understand the recording.",
        ),
      );
    }
    return response.text.trim();
...
- User's Age: ${context.age || "Not specified"}`;
    ...
    if (!response.text) {
      throw new Error(
        createErrorMessage(
          response,
          "Model failed to generate a live prompt from the audio.",
        ),
      );
    }
    return response.text.trim();
    ...
    throw new Error(
      error.message || "Failed to analyze live audio. Please try again.",
    );
  }
};
...
Create a compelling narrative that connects these moments. Use Markdown for formatting (e.g., # Title, **bold**, *italics*). The tone should be reflective and artistic. Start with a suitable title.`,
    ...
    if (!response.text) {
      throw new Error(
        createErrorMessage(response, "Model failed to generate a summary."),
      );
    }
    return response.text.trim();
  } catch (error: any) {
...
const formatPromptWithFilter = (prompt: string, filter: FilterType): string => {
  ...
  const filterStyles: Record<FilterType, string> = {
    unspecified: "",
    photorealistic:
      "ultra photorealistic style, hyper-detailed, hyper ultra realistic, real vision, realistic texture, 8k",
    Anime: "vibrant anime style, clean line art, cel shaded",
    cartoon: "charming cartoon style, bold outlines, simple shapes",
    "ink sketch":
      "black and white ink sketch style, cross-hatching, expressive lines",
    "black & white":
      "monochromatic black and white, dramatic lighting, high contrast",
    "color pop":
      "mostly black and white with a single subject in vibrant color, selective color",
    "water color":
      "soft watercolor painting style, blended colors, wet-on-wet technique",
    "raphaelite oil painting":
      "Pre-Raphaelite Brotherhood oil painting style, rich colors, high detail, romantic themes",
    "raphaelite-digital-art":
      'A beautiful blend of Pre-Raphaelite Brotherhood oil painting and vibrant Japanese Anime, creating a unique "Raphaelite Digital Art" look with rich colors, high detail, clean line art, and romantic themes.',
    "dark fantasy":
      "dark fantasy digital art, epic scale, moody lighting, style of ArtStation and DeviantArt",
    "game art":
      "AAA game art style, Unreal Engine rendering, detailed character model, dynamic pose",
    "comic cover":
      "dynamic comic book cover art, bold inks, graphic style, vibrant colors",
  };
  ...
  try {
    ...
    if (!response.candidates || response.candidates.length === 0) {
      throw new Error(
        createErrorMessage(
          response,
          "No image was generated. The prompt may have been rejected.",
        ),
      );
    }
    ...
    throw new Error(
      createErrorMessage(
        response,
        "The model generated a response, but it did not contain image data.",
      ),
    );
  } catch (error: any) {
    console.error("Error generating image:", error);
    throw new Error(
      error.message || "Failed to generate image due to an unknown error.",
    );
  }
};
...
export const editImage = async (
  ...
  try {
    ...
    if (!response.candidates || response.candidates.length === 0) {
      throw new Error(
        createErrorMessage(response, "No image was generated from the edit."),
      );
    }
    ...
    throw new Error(
      createErrorMessage(response, "Edit did not produce an image."),
    );
  } catch (error: any) {
    console.error("Error editing image:", error);
    throw new Error(
      error.message ||
        "Failed to edit image. Please try a different edit prompt.",
    );
  }
};

export const generatePromptFromAudioSegment = async (
  audioBase64: string,
  mimeType: string,
  ...
  try {
    ...
    if (!response.text) {
      throw new Error(
        createErrorMessage(
          response,
          `Model failed to generate a prompt for segment ${segmentNumber}.`,
        ),
      );
    }
    return response.text.trim();
    ...
    console.error(
      `Error generating prompt for segment ${segmentNumber}:`,
      error,
    );
    throw new Error(
      error.message || `Failed to analyze segment ${segmentNumber}.`,
    );
  }
};

export const generateStorySummaryFromAudio = async (
  audioBase64: string,
  mimeType: string,
): Promise<string> => {
  try {
    ...
    if (!response.text) {
      throw new Error(
        createErrorMessage(
          response,
          "Model failed to generate a summary for the story.",
        ),
      );
    }
    return response.text.trim();
